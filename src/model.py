...
class RewardHead(nn.Module):
    def __init__(self, hidden_size: int):
        ...
        nn.init.xavier_normal_(self.reard_layer.weight)
...
class RewardModel(nn.Module):
    ...
    def _find_reward_tokens(self, input_ids: torch.tensor, reward_token_id: int) -> torch.tensor:
        # input_ids: (B, L)
        masks = (input_ids == reward_token_id).int()  # (B, L)
        cumsums = torch.cumsum(masks, dim=1)  # (B, L)
        locs = (cumsums == 1) & masks  # (B, L)
        return torch.nonzero(locs, as_tuple=True)[1]  # (B)

    def forward(self, input_ids: torch.tensor, reward_token_id: int) -> torch.tensor:
        # input_ids: (B, L)
        # Check the location of the reward token.
        reward_locs = self._find_reward_tokens(input_ids, reward_token_id)  # (B)
        ...
        return 2 * rewards - 1  # Set the range into [-1, 1].
